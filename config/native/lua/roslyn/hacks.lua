...
function M.wrap_rpc_client(client)
    ...
    ---@private
    function result.is_closing() return client.is_closing() end

    ---@private
    function result.terminate() client.terminate() end
    ...
    function result.notify(method, params)
        if method == vim.lsp.protocol.Methods.textDocument_didChange then
            ...
            for _, change in ipairs(changes) do
                ...
                if not notified then return notified end
            end
            return true
        end
...
function M.wrap_server_cmd(cmd_fn)
    return function(...) return M.wrap_rpc_client(cmd_fn(...)) end
end

function M.fix_diagnostics_tags(diagnostics)
    for _, diagnostic in ipairs(diagnostics) do
        if diagnostic.tags ~= nil then
            diagnostic.tags = vim.tbl_filter(
                function(tag)
