---@diagnostic disable
-- START copied: vim.lsp.rpc
-- to be able to access private classes and local function
local uv = vim.uv
...
local function log_error(...)
    if log.error() then log.error(...) end
end

local function log_info(...)
    if log.info() then log.info(...) end
end

local function log_debug(...)
    if log.debug() then log.debug(...) end
end
...
local header_start_pattern = ("content"):gsub(
    "%w",
    function(c) return "[" .. c .. c:upper() .. "]" end
)
...
local default_dispatchers = {
    ...
    notification = function(method, params) log_debug("notification", method, params) end,
    ...
    on_exit = function(code, signal) log_info("client_exit", { code = code, signal = signal }) end,

    --- Default dispatcher for client errors.
    ---
    ---@param code integer Error code
    ---@param err any Details about the error
    on_error = function(code, err) log_error("client_error:", M.client_errors[code], err) end,
}
...
function M.create_read_loop(handle_body, on_no_chunk, on_error)
    ...
    return function(err, chunk)
        ...
        if not chunk then
            if on_no_chunk then on_no_chunk() end
            return
        end
...
---@return any head
---@return any|nil ...
function Client:try_call(errkind, fn, ...) return self:pcall_handler(errkind, pcall(fn, ...)) end
    ...
    elseif decoded.id and (decoded.result ~= vim.NIL or decoded.error ~= vim.NIL) then
        ...
        if decoded.error then
            ...
            if mute_error then
                ...
                if result_id and message_callbacks then message_callbacks[result_id] = nil end
                return
            end
        end
...
local function public_client(client)
    ...
    ---@private
    function result.is_closing() return client.transport.is_closing() end

    ---@private
    function result.terminate() client.transport.terminate() end
    ...
    ---@param params (table?) Parameters for the invoked LSP method
    ---@return boolean `true` if notification could be sent, `false` if not
    function result.notify(method, params) return client:notify(method, params) end
...
function M.connect(host, port)
    return function(dispatchers)
        ...
        local closing = false
        local transport = {
            write = function(msg) tcp:write(msg) end,
        ...
        tcp:connect(host, port, function(err)
            ...
            tcp:read_start(
                M.create_read_loop(
                    handle_body,
                    transport.terminate,
                    function(read_err) client:on_error(M.client_errors.READ_ERROR, read_err) end
                )
            )
        end)
...
function M.domain_socket_connect(pipe_path)
    return function(dispatchers)
        ...
        local pipe = assert(
            uv.new_pipe(false),
            string.format("pipe with name %s could not be opened.", pipe_path)
        )
        local closing = false
        local transport = {
...
function M.start(cmd, cmd_args, dispatchers, extra_spawn_params)
    ...
    local client = new_client(dispatchers, {
        write = function(msg) sysobj:write(msg) end,
        is_closing = function() return sysobj == nil or sysobj:is_closing() end,
        terminate = function() sysobj:kill(15) end,
    })

    local handle_body = function(body) client:handle_body(body) end

    local stdout_handler = M.create_read_loop(
        handle_body,
        nil,
        function(err) client:on_error(M.client_errors.READ_ERROR, err) end
    )

    local stderr_handler = function(_, chunk)
        if chunk then log_error("rpc", cmd, "stderr", chunk) end
    end

    local detached = not is_win
    if extra_spawn_params.detached ~= nil then detached = extra_spawn_params.detached end
    ...
    local ok, sysobj_or_err = pcall(vim.system, cmd1, {
        ...
        detach = detached,
    }, function(obj) dispatchers.on_exit(obj.code, obj.signal) end)
...
-- END copied: vim.lsp.rpc
---@diagnostic enable

function M.start_dynamic_socket(cmd, cmd_args, dispatchers, extra_spawn_params)
    log_info("Starting RPC client", { cmd = cmd, args = cmd_args, extra = extra_spawn_params })
    ...
    if not pipe then
        error(string.format("Failed to create pipe: %s, %s", err_name, err_msg))
        return
    end
