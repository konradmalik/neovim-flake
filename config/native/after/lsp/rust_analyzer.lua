...
vim.lsp.config(name, {
    ...
    on_attach = function(_, buf)
        vim.api.nvim_buf_create_user_command(buf, "CargoReload", function()
            reload_workspace(buf)
        end, { desc = "[" .. name .. "] Reload current cargo workspace" })

        vim.api.nvim_create_autocmd("LspDetach", {
            group = vim.api.nvim_create_augroup("personal-lsp-" .. name .. "-buf-" .. buf, { clear = true }),
            buffer = buf,
            callback = function()
                vim.api.nvim_buf_del_user_command(buf, "CargoReload")
            end,
        })
    end,
})
