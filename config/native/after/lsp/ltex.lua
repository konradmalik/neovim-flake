...
vim.lsp.config(name, {
    ...
    before_init = function()
        ...
        if not hiddenFalsePositives[current_language] then
            ...
            read_lines_into(get_false_positives_file(current_language), hiddenFalsePositives[current_language])
        end

        if not disabledRules[current_language] then
            disabledRules[current_language] = {}
            read_lines_into(get_disabled_rules_file(current_language), disabledRules[current_language])
        end
    end,
    settings = {
        ltex = {
