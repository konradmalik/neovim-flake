...
insulate("provided system values", function()
    -- generated by nix
    ...
    describe("paths", function()
        ...
        it("creates spell path properly", function()
            ...
            -- assert
            assert.are.same(tmp_dir .. "/spell", actual_path)
            assert.are.same(vim.fn.isdirectory(actual_path), 1)
        end)
        ...
        it("returns expected spell file properly", function()
            ...
            -- assert
            assert.are.same(tmp_dir .. "/spell/pl.utf-8.add", actual_path)
        end)
    end)
end)

insulate("no system values", function()
    -- generated by nix
    package.loaded["pde.system"] = {}
    ...
    vim.fn.stdpath = function(what) return tmp_dir .. "/" .. what end
    describe("paths", function()
        before_each(function() clear() end)
        after_each(function() clear() end)
    end)
    ...
    it("handles nil by returning state for notes", function()
        ...
        -- assert
        assert.are.same(tmp_dir .. "/state", actual_path)
    end)
    ...
    it("handles nil by returning state for spell", function()
        ...
        -- assert
        assert.are.same(tmp_dir .. "/state/spell", actual_path)
    end)
end)
